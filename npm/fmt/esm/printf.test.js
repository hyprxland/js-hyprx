...
test("fmt::sprintf() handles integer", function () {
  ...
  equal(sprintf("%b", -4), "-100");
  equal(sprintf("%b", 4.1), "100.0001100110011001100110011001100110011001100110011");
  equal(sprintf("%b", -4.1), "-100.0001100110011001100110011001100110011001100110011");
  equal(
...
test("fmt::sprintf() handles floatfF", function () {
  ...
  equal(sprintf("%f", -4.1), "-4.100000");
  equal(sprintf("%F", -4.1), "-4.100000");
  equal(sprintf("%f", Number.MAX_SAFE_INTEGER), "9007199254740991.000000");
  equal(sprintf("%F", Number.MAX_SAFE_INTEGER), "9007199254740991.000000");
  equal(sprintf("%f", Number.MIN_SAFE_INTEGER), "-9007199254740991.000000");
  equal(sprintf("%F", Number.MIN_SAFE_INTEGER), "-9007199254740991.000000");
...
test("fmt::sprintf() handles thorough", function () {
  tests.forEach((t, i) => {
    ...
    const should = t[2];
    equal(is, should, `failed case[${i}] : is >${is}< should >${should}<`);
  });
});
...
test("fmt::sprintf() handles formatJ", function () {
  const a = { a: { a: { a: { a: { a: { a: { a: {} } } } } } } };
  equal(sprintf("%j", a), `{"a":{"a":{"a":{"a":{"a":{"a":{"a":{}}}}}}}}`);
});
test("fmt::sprintf() handles formatI", function () {
  ...
  equal(sprintf("%I", a), "{ a: { a: { a: { a: { a: { a: { a: {} } } } } } } }");
});
...
test("fmt::sprintf() handles errors", function () {
  ...
  equal(sprintf("%.[2]*f", 1.23, "p"), `%!(BAD PREC 'p')%!(EXTRA '1.23')`);
  equal(sprintf("%.[2]*[1]f Yippie!", 1.23, "p"), "%!(BAD PREC 'p') Yippie!");
  ...
  equal(sprintf("%[hallo]s %d %d %d", 1, 2, 3, 4), "%!(BAD INDEX) 2 3 4");
  equal(sprintf("%[5]s", 1, 2, 3, 4), `%!(BAD INDEX)%!(EXTRA '2' '3' '4')`);
