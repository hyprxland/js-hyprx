...
const setHasUnion = (Set.prototype as unknown as Record<string, unknown>).union != undefined;
const setHasSymmetricDifference =
    (Set.prototype as unknown as Record<string, unknown>).symmetricDifference != undefined;
        ...
        if (a && typeof a === "object" && b && typeof b === "object") {
            ...
            if (isKeyedCollection(a) && isKeyedCollection(b)) {
                ...
                if (primitiveKeysFastPath) {
                    // enable unknown for node
                    if (a instanceof Set) {
                        if (setHasSymmetricDifference) {
                            // deno-lint-ignore no-explicit-any
                            return (a as unknown as Record<string, any>).symmetricDifference(b)
                                .size === 0;
                        }
            ...
            if (isBasicObjectOrArray(a)) {
                ...
                if (setHasUnion) {
                    // deno-lint-ignore no-explicit-any
                    keys = (getKeysDeep(a) as any).union(getKeysDeep(b));
                } else {
                    keys = new Set([...getKeysDeep(a), ...getKeysDeep(b)]);
                }
            }
