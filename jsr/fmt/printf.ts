...
import { args } from "@hyprx/process/args";
...
if (DENO) {
    platform = globals.Deno.build.os as string;
    noColorFlag = globals.Deno.noColor;
} else if (globals.process) {
    platform = globals.process.platform as string;
    if (platform === "win32") {
        platform = "windows";
    }
 ...
 * @param args The arguments to format
 */
export function printfLn(format: string, ...args: unknown[]): void {
    printf(format + EOL, ...args);
}
 ...
 * ```
 */
export function printLn(...args: string[]): void {
    if (args.length === 0) {
